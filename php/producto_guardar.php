<?php
    ...
    $factura=limpiar_cadena($_POST['producto_factura']);


    $fecha=limpiar_cadena($_POST['producto_fecha']);
    $precio=limpiar_cadena($_POST['producto_precio']);
    $arubic=limpiar_cadena($_POST['producto_arubic']);
    ...
    if($codigo=="" || $nombre=="" || $marca="" || $modelo="" || 
       $material="" || $serie="" || $estado="" || $factura="" || 
       $fecha="" || $precio=="" || $arubic="" || $ubicacion="" || 
       $stock=="" || $categoria==""){
        echo '
    ...
    if(verificar_datos("[a-zA-Z0-9- ]{1,70}",$codigo)){
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                El CODIGO de  no coincide con el formato solicitado
            </div>
        ';
        exit();
    }
    ...
    if(verificar_datos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,$#\-\/ ]{1,70}",$stock)){
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                Los comentarios no coincide con el formato solicitado
            </div>
        ';
        exit();
    }
    ...
    if($check_codigo->rowCount()>0){
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                El CODIGO ingresado ya se encuentra registrado, por favor escriba otro
            </div>
        ';
        exit();
    }
   ...
   # $check_nombre=null;
    ...
    if($check_categoria->rowCount()<=0){
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                  El resguardante seleccionado no existe 
            </div>
        ';
        exit();
    }
    ...
    $guardar_producto=$guardar_producto->prepare("INSERT INTO producto(producto_codigo,producto_nombre,producto_marca,producto_modelo,
                                                              producto_material,producto_serie,producto_estado,producto_factura,
                                                              producto_fecha,producto_precio,producto_arubic,producto_ubicacion,
                                                              producto_stock,producto_foto,categoria_id,usuario_id) 
    VALUES(:codigo,:nombre,:marca,:modelo,:material,:serie,:estado,:factura,:fecha,:precio,:arubic,
           :ubicacion,:stock,:foto,:categoria,:usuario)");
